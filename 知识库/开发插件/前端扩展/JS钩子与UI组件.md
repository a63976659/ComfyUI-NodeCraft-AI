...
创建插件时，根据以下判断是否需要在 `网页资源/` 目录下创建 JS 文件：

| 场景 | 是否需要 JS | 实现方式 |
|------|:---:|------|
| 纯计算/转换节点（接收输入→处理→输出，无特殊显示） | ❌ 不需要 | Python 节点类即可，标准 Widget 足够 |
| 需要自定义输入控件（颜色选择器、滑块、文件拖拽、画布绘制等） | ✅ 需要 | `nodeCreated` 钩子 + 自定义 Widget |
| 需要实时进度/状态反馈（长任务显示进度条、步骤提示） | ✅ 需要 | Python `send_sync` + JS `api.addEventListener` |
| 需要侧边栏 Tab 页入口 | ✅ 需要 | `registerSidebarTab` + `网页资源/` 目录 |
| 需要顶部菜单命令 | ✅ 需要 | `registerCommand` + `网页资源/` 目录 |
| 需要底部状态栏信息 | ✅ 需要 | `addStatusBarItem` + `网页资源/` 目录 |
| 节点有自定义预览/缩略图绘制需求 | ✅ 需要 | `onDrawBackground` 或 `onDrawForeground`（❗与原生控件混排时必须用占位控件预留空间，见《节点富UI前端开发》“onDrawForeground 直绘面板”章节，禁止“测量控件底部再往下画”） |
| 节点需要从 Python 推送消息到前端（执行日志、事件通知） | ✅ 需要 | `PromptServer.send_sync` + JS 事件监听 |
| 不确定是否需要 → 默认**不需要**，保持简洁 | ❌ 不需要 | 先做纯 Python 节点，后续需要再加 |

> **核心原则**：默认不加前端 JS。只有明确需要上述能力时才加。加了就要在 `__init__.py` 中设置 `WEB_DIRECTORY = "./网页资源"` 并在 `__all__` 中导出。
