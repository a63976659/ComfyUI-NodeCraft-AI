...
class 模型加载_Node:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                "Checkpoint模型": (checkpoints,),
                "加载独立VAE": ("BOOLEAN", {"default": False}),
                "VAE模型": (vaes,),
                "加载LoRA": ("BOOLEAN", {"default": False}),
                "LoRA模型": (loras,),
                "LoRA强度": (
                    "FLOAT",
    ...
    RETURN_NAMES = ("模型", "CLIP", "VAE", "模型信息")
    FUNCTION = "load_model"
    CATEGORY = "🔧 自定义工具/模型加载"
    DESCRIPTION = "加载 Checkpoint/VAE/LoRA 模型，支持动态模型列表、路径校验与设备自动选择。"
    ...
    def load_model(
        ...
        # ── 1. 加载 checkpoint ──────────────────────────────
        ckpt_path = _resolve_model_path("checkpoints", Checkpoint模型)
        cache_key = f"ckpt:{ckpt_path}"

        if cache_key in _MODEL_CACHE:
            model_data = _MODEL_CACHE[cache_key]
            logger.info("从缓存加载 checkpoint: %s", Checkpoint模型)
        else:
            ...
            model_data = load_checkpoint_guess_config(
                ckpt_path,
                output_vae=not 加载独立VAE,  # 如果要加载独立 VAE，则不加载 checkpoint 内置 VAE
                output_clip=True,
                embedding_directory=folder_paths.get_folder_paths("embeddings"),
            )
            _MODEL_CACHE[cache_key] = model_data
            logger.info("成功加载 checkpoint: %s (设备: %s)", Checkpoint模型, device)

        model, clip, vae, clipvision = model_data[:4]
        info_parts.append(f"Checkpoint: {Checkpoint模型}")

        # ── 2. 可选：加载独立 VAE ────────────────────────────
        if 加载独立VAE and VAE模型 and VAE模型 != "(无可用模型)":
            vae_path = _resolve_model_path("vae", VAE模型)
            ...
            info_parts.append(f"VAE: {VAE模型}")
            logger.info("成功加载 VAE: %s", VAE模型)

        # ── 3. 可选：加载 LoRA ───────────────────────────────
        if 加载LoRA and LoRA模型 and LoRA模型 != "(无可用模型)":
            lora_path = _resolve_model_path("loras", LoRA模型)
            from comfy.sd import load_lora_for_models

            model, clip = load_lora_for_models(
                model, clip, lora_path, LoRA强度, LoRA强度
            )
            info_parts.append(f"LoRA: {LoRA模型} (强度: {LoRA强度})")
            logger.info("成功加载 LoRA: %s (强度: %.2f)", LoRA模型, LoRA强度)
    ...
    def IS_CHANGED(
        ...
        **kwargs,
        ...
        """模型选择变化时需要重新加载"""
        return f"{Checkpoint模型}|{加载独立VAE}|{VAE模型}|{加载LoRA}|{LoRA模型}|{LoRA强度}"
