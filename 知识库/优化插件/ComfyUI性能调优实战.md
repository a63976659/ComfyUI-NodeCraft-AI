...
本文档涵盖 ComfyUI 插件开发中的性能调优实践，从 GPU 显存管理到推理加速，从内存泄漏防护到基准测试方法。每个主题均附带可直接参考的代码示例。
...
def profile_node(node_class, *args, **kwargs):
    """使用 cProfile 分析节点的执行瓶颈"""
    profiler = cProfile.Profile()
    profiler.enable()
    node_class.execute(*args, **kwargs)
    profiler.disable()

    stats = pstats.Stats(profiler)
    stats.sort_stats("cumulative")
    stats.print_stats(20)  # 打印耗时最多的 20 个函数

    # 也可以保存到文件用 snakeviz 可视化
    stats.dump_stats("node_profile.prof")
```

### 5.3 torch.profiler 深度分析
