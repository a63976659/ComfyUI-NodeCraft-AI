...
# 直接触发规则 — 命中即直接返回"需要规划"，不走打分（中英双语，保证英文界面下同样可触发）
_直接触发词 = [
    "写个计划", "写计划", "制定计划", "做个规划", "规划一下",
    "规划任务", "制定方案", "写个方案", "出个方案", "先规划",
    "先计划", "列个计划", "帮我规划", "帮我计划",
    "plan first", "plan this", "plan the task", "make a plan",
    "create a plan", "outline a plan", "propose a plan",
]
...
6. **执行计划**：列出具体执行步骤（最多5步，每步一句话）：
   - 新增/修改节点 Python 文件的步骤必须指明放入 `节点/` 目录（英文项目为 `nodes/`，以项目实际目录为准），严禁放在插件根目录
   - 如果 needs_frontend 为 true，计划中必须包含在 `网页资源/` 目录（英文项目为 `web/`）创建 JS 文件的步骤
