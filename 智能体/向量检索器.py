...
设计目标：
  ...
  - 中文 bigram 双字分词 + 英文按词分词，覆盖中英文混合文档
...
class TFIDF检索器:
    ...
    def _分词(self, text: str) -> list:
        ...
        英文：按空格和标点分词，转小写
        中文：bigram 双字分词（P2-13：降低单字噪音，与 BM25 对齐），
              无 bigram 时（单字输入）回退逐字
        ComfyUI 术语：整词匹配，避免重要术语被切碎

        示例："ComfyUI 节点开发" → ["comfyui", "节点", "点开", "开发", "节点"]
        """
        if not text:
            return []
        ...
        # 中文 bigram（单字仅在无 bigram 时回退，避免单字噪音稀释权重）
        chinese_chars = re.findall(r'[\u4e00-\u9fff]', text)
        if len(chinese_chars) >= 2:
            tokens.extend(chinese_chars[i] + chinese_chars[i + 1]
        ...
        if cache_path is not None:
            try:
                ...
                if cache_data.get("tokenizer_version") != _TOKENIZER_VERSION:
                    raise KeyError(
                        f"分词器版本不匹配（缓存 {cache_data.get('tokenizer_version')} ≠ 当前 {_TOKENIZER_VERSION}）"
                    )
