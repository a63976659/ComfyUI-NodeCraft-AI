...
class 跨会话记忆管理器:
    ...
    def 删除单条记忆(self, 类型: str, 索引: int, 插件路径: str = None) -> bool:
        ...
        - 类型: "用户偏好" / "常见问题" / "上下文" / "plugin"
        - 索引: 在列表中的位置（类型为 "plugin" 时忽略）
        - 插件路径: 对 "上下文" 和 "plugin" 类型有效（可为插件名或完整路径）
        """
        with self._lock:
            data = self._加载记忆()
            ...
            if 类型 == "用户偏好":
                ...
                if 0 <= 索引 < len(问题列表):
                    ...
                    self._保存记忆()
                    return True
            elif 类型 in ("上下文", "plugin") and 插件路径:
                插件名 = self._提取插件名(插件路径)
                if 插件名 and 插件名 in data["插件记忆"]:
                    if 类型 == "plugin":
                        # 删除整个插件记忆节点（项目信息 + 全部上下文）
                        del data["插件记忆"][插件名]
                        self._保存记忆()
                        logger.info(f"删除插件记忆: {插件名}")
                        return True
