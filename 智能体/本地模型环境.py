...
_主环境site_packages = Path(sys.executable).parent / "Lib" / "site-packages"
...
# venv 内部路径（Windows 约定为 Scripts/python.exe）
_venv_python = _venv根目录 / "Scripts" / "python.exe"
_pth文件 = _venv根目录 / "Lib" / "site-packages" / "_comfyui_packages.pth"
_路径修正模块 = _venv根目录 / "Lib" / "site-packages" / "_nodecraft_path_fix.py"
...
def 获取venv_python() -> Path:
    """返回 venv 中的 python.exe 路径。

    Returns:
        Windows 下为 ``Scripts/python.exe`` 的绝对路径。
    """
    return _venv_python
