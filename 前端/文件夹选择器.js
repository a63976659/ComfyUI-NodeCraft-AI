...
import {
    el, NCA_STORAGE_KEYS, 插件存储键, registerCleanup, trackPanelTimer, removePanelTimer,
} from "./工具函数.js";
 ...
 * @param {string} [scope] - 界面作用域 'develop' | 'optimize' | 'visualize'，决定读写哪个插件键与事件过滤
 * @returns {HTMLElement} 组件 DOM 元素
 */
export function 创建文件夹选择器(onSelect, 获取插件路径, onReviewClick, scope = 'develop') {
    let 当前列表 = [];
    let 当前显示列表 = [];
    let 选中项 = localStorage.getItem(插件存储键(scope));
    ...
    const 标题文本 = el("span", { class: "nc-folder-selector-title", text: t("folder.select_title") });
    ...
    if (typeof onReviewClick === "function") {
        ...
        reviewBtn = el("button", {
            ...
            title: t("folder.review_title"),
            html: '<span class="nca-review-icon">🔍</span> ' + t("optimize.code_review"),
        });
    ...
    // 外部装载按钮
    const 外部装载处理 = () => {
        const path = prompt(t("folder.external_prompt"));
        if (path && path.trim()) {
            选中项 = path.trim();
            设置插件文件夹(选中项, scope);
            更新已选名称(选中项);
    ...
    // 订阅全局插件选择变更事件：仅同步本作用域的高亮状态（仅更新选中样式，不触发 onSelect 避免走业务加载）
    // payload 为 { scope, value } 对象；非本作用域的变更一律忽略，保证三界面互不干扰。
    const 插件变更处理 = (payload) => {
        const ev = (payload && typeof payload === 'object') ? payload : { scope: 'develop', value: payload };
        if ((ev.scope || 'develop') !== scope) return;
        const 新值 = ev.value || '';
        if (选中项 === 新值) return;
        选中项 = 新值;
        更新已选名称(新值);
