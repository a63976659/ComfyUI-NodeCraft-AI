...
const 附件最大数 = 6;
// 音频仅本地多模态模型（如 Gemma 4 12B/E 系）支持，后端会做能力前置检查
const 附件接受类型 = "image/png,image/jpeg,image/gif,image/webp,.txt,.py,.js,.json,.md,.css,.html,.yaml,.yml,.toml,.cfg,.ini,.sh,.bat,.wav,.mp3,.m4a,.ogg,.flac";
...
export function 创建附件组件(rootContainer, onChange) {
    ...
    const 文件按钮 = el("button", { class: "nca-attach-btn", html: "+", title: t("chat.attach") });
    ...
    async function 更新附件按钮状态() {
        try {
            ...
            if (!能力.supports_vision && !能力.supports_file_content) {
                文件按钮.disabled = true;
                文件按钮.title = t("chat.attach_unsupported");
                文件按钮.classList.add("nca-attach-disabled");
            } else {
                文件按钮.disabled = false;
                文件按钮.title = t("chat.attach");
    ...
    文件输入.addEventListener("change", (e) => {
        ...
        if (文件列表.length + files.length > 附件最大数) {
            Toast.warning(t("chat.attach_max", { max: 附件最大数 }));
            文件输入.value = "";
            return;
        }
        // 检测是否含图片，触发视觉能力警告
