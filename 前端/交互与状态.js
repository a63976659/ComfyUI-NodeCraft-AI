...
/**
 * 调用本地计费代理进行 RanKing 扣款。
 * 返回值：{ success: boolean, error?: string, code?: string, ... }
 *   - code === "INSUFFICIENT_BALANCE"：余额不足，需阻断发送
 *   - 其他失败（未登录 / 服务不可用 / 网络错误）：不阻断发送
 */
async function 扣款(amount = AI_CHAT_COST, reason = "nodecraft_ai_chat", referenceId) {
    const token = _获取Token() || "";
    if (!token) return { success: false, error: "未登录", code: "NOT_LOGGED_IN" };

    try {
        const resp = await fetch("/nca/billing/deduct", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`,
                "X-CSRF-Token": _获取或生成CsrfToken(),
                "X-Idempotency-Key": referenceId || `msg_${Date.now()}_${_生成幂等键()}`,
            },
            body: JSON.stringify({
                amount,
                reason,
                reference_id: referenceId || `msg_${Date.now()}`,
            }),
        });
        let data = null;
        ...
        if (data && data.success === false) {
            return { success: false, error: data.error || data.message || "扣款失败", code: data.code || "DEDUCT_FAILED", ...data };
        }
        return { success: true, ...(data || {}) };
    } catch (e) {
        return { success: false, error: e?.message || String(e), code: "NETWORK_ERROR" };
    }
}
...
export async function 发送消息(content, attachments = []) {
    ...
    // ── 发送前 RanKing 扣款（余额不足则阻断；计费服务故障不阻断）──
    const 扣款结果 = await 扣款(AI_CHAT_COST, "nodecraft_ai_chat", 状态.当前会话ID || undefined);
    if (!扣款结果.success && 扣款结果.code === "INSUFFICIENT_BALANCE") {
        try { Toast.warning("余额不足，请在 RanKing 中充值"); } catch (_) {}
        状态.正在发送 = false;
        事件总线.emit(事件.发送状态变更, false);
        事件总线.emit(事件.状态栏更新, "余额不足");
        状态.连接状态 = "online";
        事件总线.emit(事件.连接状态变更, "online");
        return null;
    }
    if (!扣款结果.success) {
        // 未登录 / 服务不可用 / 网络错误：记日志但不阻断发送，优先保证可用性
        console.warn("[节点梦工厂] 计费扣款未成功，继续发送：", 扣款结果);
    }
...
export async function 更新模型选择({ 模型来源, 选中本地模型 } = {}) {
