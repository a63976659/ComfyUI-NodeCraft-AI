/**
 * @module 工具函数
 * @description NodeCraft AI 前端公共工具库
 *
 * 提供全局常量、DOM 工厂、事件清理注册表、防抖、Markdown 渲染、
 * Toast 通知、快捷键管理、图片压缩等基础能力。
 *
 * 模块区域划分：
 * - 常量与配置：存储键、颜色、工具名映射
 * - 主题切换：深色/浅色模式管理
 * - DOM 工具：元素创建、HTML 转义
 * - 事件清理注册表：防止侧边栏重建时事件泄漏
 * - 防抖与定时器：延迟执行与生命周期管理
 * - Markdown 渲染：marked.js 高性能渲染 + 正则降级
 * - XSS 消毒：DOMPurify + 本地白名单降级
 * - Toast 通知：多类型堆叠通知
 * - 快捷键：全局组合键注册与分发
 * - 图片与附件：压缩、缩略图、视觉能力检测
 */
...
export const NCA_STORAGE_KEYS = {
    plugin: 'NodeCraftAI_selectedPlugin',            // 开发(develop)界面，沿用历史键名保证兼容
    pluginOptimize: 'NodeCraftAI_selectedPlugin_optimize',   // 优化(optimize)界面独立键
    pluginVisualize: 'NodeCraftAI_selectedPlugin_visualize', // 可视化(visualize)界面独立键
    session: 'NodeCraftAI_sessions',
    activeTab: 'NodeCraftAI_activeTab',
    theme: 'NodeCraftAI_theme',
};

// 按界面作用域映射到对应的插件文件夹存储键。
// 三个主要工作界面（develop/optimize/visualize）各自使用独立键，互不干扰。
// develop 与历史行为保持一致沿用 plugin 键；未知 scope 一律回退到 develop。
export function 插件存储键(scope) {
    if (scope === 'optimize') return NCA_STORAGE_KEYS.pluginOptimize;
    if (scope === 'visualize') return NCA_STORAGE_KEYS.pluginVisualize;
    return NCA_STORAGE_KEYS.plugin;
}
...
export const 工具名映射 = {
    ...
    "grep_plugin_files": "跨文件搜索",
    "find_plugin_files": "查找文件",
    "rename_plugin_file": "重命名文件",
    "delete_plugin_file": "删除文件",
    "check_python_syntax": "Python 语法检查",
    "execute_command": "执行命令",
    "update_readme": "更新README",
    "update_plan": "更新任务计划",
    "analyze_profiling": "性能分析",
};

// 将英文工具名转为界面语言显示名（i18n 词典 tool.* 键）；未收录时原样返回
export function 工具名显示(工具名) {
    return 工具名映射[工具名] ? t('tool.' + 工具名) : 工具名;
}
...
// 区域：事件清理注册表
// ────────────────────────────────────────────────────────────
// 侧边栏面板在每次重建时会重新绑定事件，若不清理旧的监听器将导致
// 内存泄漏和重复触发。本注册表统一记录所有通过 registerCleanup()
// 注册的事件监听，并在面板销毁时一键移除（cleanupAllEvents()）。
// 使用流程：
//   const key = registerCleanup(btn, 'click', handler);
//   // ... 面板销毁时 ...
//   cleanupAllEvents(); // 或 removeCleanup(key) 单独移除
// ============================================================
const _eventCleanups = new Map();
 ...
 * @param {string|null} [key=null] - 可选唯一标识符，用于后续 removeCleanup
 * @returns {string} 注册键，可传给 removeCleanup() 单独移除
 */
export function registerCleanup(element, event, handler, key = null) {
    if (window.__NCA_DEBUG__) {
        console.debug(`[NCA-Event] 注册: ${event} on`, element, key ? `(key: ${key})` : '');
    }
    element.addEventListener(event, handler);
    const k = key || `${event}_${_eventCleanups.size}_${Date.now()}`;
    _eventCleanups.set(k, { element, event, handler });
    return k;
}

export function removeCleanup(key) {
    if (window.__NCA_DEBUG__) {
        console.debug(`[NCA-Event] 移除: key=${key}`);
    }
    const item = _eventCleanups.get(key);
    if (item) {
        item.element.removeEventListener(item.event, item.handler);
        _eventCleanups.delete(key);
    }
}

export function cleanupAllEvents() {
    if (window.__NCA_DEBUG__) {
        console.debug(`[NCA-Event] 清理全部: ${_eventCleanups.size} 个监听`);
    }
...
/**
 * 创建防抖函数（trailing 模式）
 * @param {Function} fn - 被防抖的原始函数
 * @param {number} [delayMs=300] - 防抖延迟毫秒数
 * @returns {Function & {cancel: Function}} 防抖后的函数，附带 cancel() 方法
 */
export function createDebounce(fn, delayMs = 300) {
    let tid = null;
    const debounced = (...args) => {
        clearTimeout(tid);
        tid = setTimeout(() => fn(...args), delayMs);
    };
    debounced.cancel = () => { clearTimeout(tid); tid = null; };
    return debounced;
}

// ─── 面板定时器追踪（增强版）──────────────────────────────────
const _panelTimers = new Map(); // id -> { createdAt, name }

export function trackPanelTimer(id, name = 'unnamed') {
    _panelTimers.set(id, { createdAt: Date.now(), name });
    return id;
}

export function clearPanelTimers() {
    _panelTimers.forEach((meta, id) => clearTimeout(id));
    _panelTimers.clear();
}

export function removePanelTimer(id) {
    if (_panelTimers.has(id)) {
        clearTimeout(id);
        _panelTimers.delete(id);
    }
}

// 清理超时的定时器（超过60秒未清理的视为泄漏）
export function cleanupStaleTimers(maxAgeMs = 60000) {
    const now = Date.now();
    _panelTimers.forEach((meta, id) => {
...
/**
 * DOM 元素工厂函数——快速创建并配置 HTML 元素
 * @param {string} tag - 标签名
 * @param {Object} [attrs={}] - 属性对象，支持 class/style/on事件/html/text 等快捷键
 * @param {Array|HTMLElement|string} [children=[]] - 子节点数组或单个子节点
...
/**
 * 禁止外部翻译改写——给本插件根级容器加翻译豁免标记
 * - translate="no" + class notranslate：浏览器翻译（Chrome/Edge/Google 翻译）通用豁免；
 * - class lite-search-item-type：ComfyUI-Chinese-Translation 翻译插件 excludeClass 黑名单类，
 *   其 tSkip() 用 closest() 匹配，祖先带该类即整棵子树跳过翻译改写；
 *   宿主前端 CSS 中该类仅存在复合选择器 .litegraph.lite-search-item-type（需同时带
 *   litegraph 类才生效），单独加此类无任何样式副作用。
 * 仅作用于本插件自建的根 DOM，不影响宿主与其它插件。
 * @param {HTMLElement} node - 需要豁免翻译的根容器
 * @returns {HTMLElement} 原样返回该容器（便于链式使用）
 */
export function 标记禁止翻译(node) {
    if (!node || !node.setAttribute) return node;
    node.setAttribute("translate", "no");
    node.classList.add("notranslate", "lite-search-item-type");
    return node;
}

/**
 * 注入关键伪元素样式——JS 直接向 document.head 挂 <style>（双保险）
 * 背景：@import 链加载的 CSS 文件中同名规则在浏览器端曾出现未进 CSSOM 不渲染的
 * 异常，而 JS 注入的 <style> 同规则立即生效。CSS 文件中的规则保留，此处逐字
 * 重复一份，双重声明结果一致无渲染差异。
 * 幂等：以固定 id 查重，已存在即返回。
 */
export function 注入关键样式() {
    if (document.getElementById("nca-critical-label-css")) return;
    const style = document.createElement("style");
    style.id = "nca-critical-label-css";
    style.textContent = `
.nc-tab::before { content: attr(data-nca-label); }
.nca-switch-btn::before { content: attr(data-nca-label); }
.nca-unload-btn::before { content: attr(data-nca-label); }
.nca-viz-mode-btn::before { content: attr(data-nca-label); }
`;
    document.head.appendChild(style);
}

// ============================================================
// 区域：Markdown 渲染与 XSS 消毒
// ============================================================
// HTML 转义工具
...
const _本地允许属性 = new Set(['href','src','alt','title','class','id','target','rel','open','disabled','loading']);
...
function _配置Marked() {
    ...
    // 图片懒加载
    _markedRenderer.image = function(href, title, text) {
        if (typeof href === 'object' && href !== null) {
            title = href.title;
            text = href.text;
            href = href.href;
        ...
        const t = title ? ` title="${_转义HTML(title)}"` : '';
        const alt = text ? ` alt="${_转义HTML(text)}"` : '';
        return `<img src="${_转义HTML(href || '')}"${alt}${t} loading="lazy">`;
    };
...
export const Toast = {
    ...
    _getContainer() {
        if (!this._container || !this._container.parentNode) {
            ...
            // Toast 容器可能直挂 document.body（sidebarRoot 防护覆盖不到），单独加翻译豁免
            标记禁止翻译(this._container);
            // 优先挂载到侧边栏根容器内部
...
export async function 显示视觉能力警告(inputArea) {
    ...
    const banner = el("div", { class: "nca-vision-warning" }, [
        el("span", { text: t("chat.vision_warning") }),
...
/**
 * 为用户消息气泡追加附件缩略图（异步压缩，不阻塞渲染）
 * 图片附件（含 base64 data）→ 缩略图；非图片附件或无 data 的历史附件 → 扩展名图标+文件名卡片
...
export async function 追加附件缩略图(msgBody, attachments) {
    ...
    for (const att of attachments) {
        const 是图片 = _附件图片类型.includes(att.type);
        if (是图片 && att.data) {
            const thumb = el('img', { class: 'nca-msg-attach-thumb', title: att.name || '' });
            ...
            待压缩.push({ thumb, data: att.data });
        } else {
            // 非图片附件或历史消息（后端仅持久化 name/type/size）→ 文件名卡片（复用 输入与附件.css 样式类）
            const ext = ((att.name || '').split('.').pop() || 'FILE').toUpperCase().slice(0, 6);
            container.appendChild(el('div', { class: 'nca-attach-card', title: att.name || '' }, [
                el('div', { class: 'nca-attach-icon', text: 是图片 ? 'IMG' : ext }),
                el('span', { class: 'nca-attach-name', text: att.name || t('chat.attachment') }),
            ]));
        }
    }
    ...
    // 异步压缩图片缩略图
    for (const { thumb, data } of 待压缩) {
        try {
            thumb.src = await 压缩图片缩略(data);
        } catch (_) { /* 保留原图 */ }
    }
}
...
// ─── 消息行内编辑公共脚手架 ─────────────────────────────────
/**
 * 在消息气泡内构建行内编辑器（主聊天与面板会话共用）：
 * textarea + 取消/确认按钮 + 自适应高度 + ESC 取消 / Ctrl(Cmd)+Enter 确认。
 * 差异逻辑由回调注入：
 * @param {HTMLElement} msgEl - 消息气泡元素（含 .nca-msg-body）
 * @param {string} 初始内容 - textarea 初始文本
 * @param {Object} callbacks
 * @param {Function} [callbacks.onCancel] - 取消恢复原内容后的额外处理 (msgEl) => void
 * @param {Function} callbacks.onConfirm - 非空校验通过后的确认处理 (newContent, { body, originalHTML }) => void
 */
export function 创建行内编辑器(msgEl, 初始内容, { onCancel, onConfirm }) {
    ...
    const cancelBtn = el("button", { class: "nca-edit-cancel", text: t("common.cancel") });
    const confirmBtn = el("button", { class: "nca-edit-confirm", text: t("chat.confirm_regen") });
    ...
    cancelBtn.addEventListener("click", () => {
        ...
        if (onCancel) onCancel(msgEl);
    });

    confirmBtn.addEventListener("click", () => {
        const newContent = textarea.value.trim();
        if (!newContent) {
            try { Toast.warning(t("chat.empty_content")); } catch (_) {}
            return;
        }
...
// 区域：快捷键管理
// ============================================================
const _shortcuts = new Map(); // key combo -> { handler, description }

export function registerShortcut(combo, handler, description = '') {
    _shortcuts.set(combo.toLowerCase(), { handler, description });
}
...
export function getRegisteredShortcuts() {
    return [..._shortcuts.entries()].map(([combo, { description }]) => ({ combo, description }));
}

// 全局快捷键监听（初始化一次）
let _shortcutListenerActive = false;
export function initShortcutListener() {
    if (_shortcutListenerActive) return;
    _shortcutListenerActive = true;
