...
export function 创建文件编辑器面板(options = {}) {
    ...
    // ─── 事件监听 ──────────────────────────────────────────────
    // payload 现为 { scope, value } 对象（旧版本为字符串），此处做兼容归一化
    function 处理插件选择变更(payload) {
        const pluginName = (payload && typeof payload === 'object') ? (payload.value || '') : (payload || '');
        当前文件夹 = pluginName || "";
        // 重置编辑状态
        当前文件路径 = "";
        原始内容 = "";
