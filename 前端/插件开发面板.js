...
export function 创建模型切换栏副本(ctx) {
    ...
    const reasonBtn = el("button", { class: "nca-unload-btn nca-reasoning-btn", title: t("model.reasoning_title") });
    function 刷新思考按钮() {
        const 支持 = 状态.模型来源 === "api" && 支持思考深度(状态.设置.model_name);
        reasonBtn.style.display = 支持 ? "" : "none";
        if (!支持) return;
        const 档位 = 当前思考深度();
        reasonBtn.dataset.ncaLabel = `${t("model.reasoning")}: ${档位 || t("model.reasoning_default")}`;
        reasonBtn.classList.toggle("active", !!档位);
    }
    bar.appendChild(lSel); bar.appendChild(unloadBtn); bar.appendChild(aSel); bar.appendChild(reasonBtn);
    ...
    reasonBtn.addEventListener("click", async () => {
        if (reasonBtn.disabled) return;
        reasonBtn.disabled = true;
        try { await 切换思考深度(); } finally { reasonBtn.disabled = false; }
        刷新思考按钮();  // 设置已保存事件也会刷新，此处确保持久化失败时标签也即时回显
    });
