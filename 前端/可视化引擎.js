...
// Three.js 与 OrbitControls 按需动态加载：仅在首次实际创建 3D 场景前 await 加载，
// 避免打开侧边栏即加载 ~2.5MB 的 three.module.js
let THREE = null;
let OrbitControls = null;
let libLoaded = false;
let _加载Promise = null;   // 模块级缓存加载 promise，防止并发重复加载
...
// 连线状态权重：红 > 黄 > 绿，连线取「两端节点 + 边自身」中最危险的一档着色
const 状态权重 = { normal: 0, warning: 1, error: 2 };

/** 返回若干状态中权重最高（最危险）的那个 */
function 取最危险状态(...状态列表) {
    return 状态列表.reduce(
        (累计, s) => ((状态权重[s] || 0) > (状态权重[累计] || 0) ? s : 累计),
        'normal',
    );
}
...
/**
 * 异步加载 Three.js + OrbitControls（动态 import，本地 lib 目录）
 * 模块级缓存 promise：多处并发调用只触发一次实际加载。
 * @param {Function} [onStart] - 首次实际发起加载时调用（兼容旧回调）
 */
...
export function 创建可视化图(container, graphData, options = {}) {
    if (!THREE || !OrbitControls) {
        console.error('[节点梦工厂] Three.js 未加载，请先 await 加载可视化库()');
        return null;
    }
    ...
    edges.forEach(e => {
        const a = nodeMap[e.source], b = nodeMap[e.target];
        if (!a || !b) return;
        // 连线颜色取两端节点与边自身状态中最危险的一档：
        // 红红 / 红黄 / 红绿 → 红，黄黄 / 黄绿 → 黄，仅绿绿 → 绿
        e._显示状态 = 取最危险状态(e.status, a.status, b.status);
        const isErr = e._显示状态 === 'error', isWarn = e._显示状态 === 'warning';
    ...
    function unhoverNode(n) {
        ...
        edges.forEach(e => {
            if (e.source === n.id || e.target === n.id) {
                const isE = e._显示状态 === 'error', isW = e._显示状态 === 'warning';
    ...
    graphInstance._destroy = function () {
        ...
        // 移除事件监听（canvas 交互 + OrbitControls 内部监听 + 容器尺寸监听）
        renderer.domElement.removeEventListener('mousemove', onMouseMove);
        renderer.domElement.removeEventListener('click', onClick);
        controls.dispose();
        resizeObserver.disconnect();

        // 递归释放场景内全部 geometry / material（含数组材质与各类贴图 texture）
        const 贴图字段 = ['map', 'normalMap', 'emissiveMap', 'roughnessMap', 'metalnessMap',
            'aoMap', 'alphaMap', 'envMap', 'lightMap', 'bumpMap', 'displacementMap', 'specularMap', 'gradientMap'];
        const 释放材质 = (material) => {
            if (!material) return;
            贴图字段.forEach(key => {
        ...
        scene.traverse(obj => {
            ...
            if (obj.material) {
                if (Array.isArray(obj.material)) obj.material.forEach(释放材质);
                else 释放材质(obj.material);
            }
        });
        scene.clear();

        // 释放渲染器并强制丢弃 WebGL 上下文
        renderer.dispose();
        if (typeof renderer.forceContextLoss === 'function') {
            try { renderer.forceContextLoss(); } catch (_) {}
        }

        // 移除 canvas，清空对 DOM / Three.js 对象的引用，便于 GC
        if (renderer.domElement.parentNode) {
            renderer.domElement.parentNode.removeChild(renderer.domElement);
        }
        nodeMeshes.length = 0;
        glowSprites.length = 0;
        nodes.forEach(n => { n._mesh = null; n._glow = null; n._label = null; });
        edges.forEach(e => { e._line = null; e._tube = null; });
        Object.keys(nodeDataMap).forEach(k => { delete nodeDataMap[k]; });
        hoveredNode = null;
        selectedNode = null;
        graphInstance._scene = null;
        graphInstance._camera = null;
        graphInstance._renderer = null;
        graphInstance._nodes = null;
        graphInstance._edges = null;
        graphInstance._nodeMap = null;
    };
    // 完整销毁入口（与 _destroy 同一实现，供外部直接调用）
    graphInstance.销毁 = graphInstance._destroy;
