...
// ─── 颜色 / 尺寸映射 ─────────────────────────────────────────
const STATUS_COLORS = {
    normal:  { main: 0x00ff88, emissive: 0x00aa55, glow: '#00ff88' },
    error:   { main: 0xff2255, emissive: 0xcc0033, glow: '#ff2255' },
    warning: { main: 0xffaa00, emissive: 0xcc7700, glow: '#ffaa00' },
};
...
export function 创建可视化图(container, graphData, options = {}) {
    ...
    const dLight = new THREE.DirectionalLight(0x88bbff, 1.5);
    dLight.position.set(50, 80, 60);
    scene.add(dLight);
    const dLight2 = new THREE.DirectionalLight(0x443366, 0.8);
    dLight2.position.set(-40, -20, -50);
    scene.add(dLight2);
    const pLight = new THREE.PointLight(0x00e5ff, 1.0, 300);
    scene.add(pLight);
    ...
    const stars = new THREE.Points(starGeo, new THREE.PointsMaterial({
        color: 0x3355aa, size: 0.6, transparent: true, opacity: 0.5,
        sizeAttenuation: true, blending: THREE.AdditiveBlending, depthWrite: false,
    }));
    scene.add(stars);

    // ── 网格 ──
    const grid = new THREE.GridHelper(300, 60, 0x0a1a30, 0x0a1a30);
    grid.position.y = -50;
    ...
    nodes.forEach(n => {
        ...
        // 文字标签
        const { texture, aspect } = createLabelTexture(n.name || n.id);
    ...
    edges.forEach(e => {
        ...
        const color = isErr ? 0xff2255 : isWarn ? 0xffaa00 : 0x00ff88;
    ...
    function unhoverNode(n) {
        if (!n) return;
        n._mesh.material.emissiveIntensity = 0.6;
        n._mesh.scale.set(1, 1, 1);
        n._glow.material.opacity = 0.7;
        edges.forEach(e => {
            if (e.source === n.id || e.target === n.id) {
