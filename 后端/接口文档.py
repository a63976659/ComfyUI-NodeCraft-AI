    ...
    "paths": {
        ...
        "/sessions/{id}/messages": {
            "get": {
                "tags": ["会话管理"],
                "summary": "获取会话消息",
                "description": "获取指定会话的消息记录。无任何参数时全量返回所有消息（旧契约）；?recent=1 窗口模式返回最近 50 条（首屏性能优化）；?after_index=N 增量返回数组索引大于 N 的消息（游标为消息在会话中的数组索引）；?page&page_size 显式分页按页切片。响应始终携带 total 与 start_index（返回片段首条消息在全量列表中的绝对索引，供编辑重生成等场景换算服务端索引）。",
                "parameters": [
                    {"name": "id", "in": "path", "required": True, "schema": {"type": "string"}, "description": "会话 UUID"},
                    {"name": "recent", "in": "query", "required": False, "schema": {"type": "string", "enum": ["1"]}, "description": "窗口模式：为 1 时仅返回最近 50 条消息（与 after_index/page 互斥，优先级最高）"},
                    {"name": "after_index", "in": "query", "required": False, "schema": {"type": "integer"}, "description": "增量游标：返回数组索引大于 N 的消息，最多 page_size 条"},
                    {"name": "page", "in": "query", "required": False, "schema": {"type": "integer", "minimum": 1}, "description": "显式分页页码（从 1 开始）"},
                    {"name": "page_size", "in": "query", "required": False, "schema": {"type": "integer", "default": 50, "maximum": 100}, "description": "每页条数（默认 50，最大 100）"}
                ],
                "responses": {
                    "200": {"description": "成功", "content": {"application/json": {"schema": {"type": "object", "properties": {
                        "messages": {"type": "array", "items": {"type": "object", "properties": {"role": {"type": "string", "enum": ["user", "assistant"]}, "content": {"type": "string"}, "timestamp": {"type": "string"}}}},
                        "total": {"type": "integer", "description": "会话消息总数（所有模式均返回）"},
                        "start_index": {"type": "integer", "description": "返回片段首条消息在全量列表中的绝对索引（无参数全量模式恒为 0，所有模式均返回）"},
                        "page": {"type": "integer", "description": "当前页码（分页模式）；recent 模式为最后一页页码；全量/after_index 模式不返回"},
                        "page_size": {"type": "integer", "description": "页大小（分页/recent 模式返回）"},
                        "total_pages": {"type": "integer", "description": "总页数（分页/recent 模式返回）"},
                        "has_more": {"type": "boolean", "description": "是否还有更新的消息（仅 after_index 增量模式返回）"}
                    }}}}},
                    "400": {"description": "after_index 非整数"},
                    "401": {"description": "未认证"},
        ...
        "/ws": {
            "get": {
                ...
                "description": "建立 WebSocket 实时通信连接。支持消息类型：chat（对话）、ping（心跳）。响应类型：status、chunk、done、error、pong。",
                "responses": {
