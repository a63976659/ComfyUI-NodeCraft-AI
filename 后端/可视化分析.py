...
from .系统环境映射 import get_plugin_root, get_default_llm_path
...
def collect_code_summary(plugin_path: Path) -> tuple:
    ...
    - file_tree_str: 类似 tree 命令的目录结构
    - code_summary_str: 拼接所有 .py/.js 摘要（带文件路径分隔）
    """
...
输出格式：
{{"nodes": [{{"id": "唯一标识", "name": "模块名称", "type": "类型", "description": "功能描述", "group": "所属分组"}}], "links": [{{"source": "源节点id", "target": "目标节点id", "label": "关系描述"}}], "stats": {{"total_count": 数量}}}}
...
async def _call_ai_for_function_mode(prompt: str, llm_client, local_model_client) -> str:
    ...
    if model_source == "local" and local_model_client is not None:
        ...
        async for chunk in local_model_client.流式对话(messages, settings):
            full_text += chunk
    else:
        if llm_client is None:
            raise RuntimeError("API 模型客户端未初始化")
        async for chunk in llm_client.流式对话(messages, settings):
            full_text += chunk
        ...
        try:
            ...
            if 云端计费:
                ...
                if cost_credits > 0:
                    logger.info(f"[可视化计费] 扣费={cost_credits}积分")
        except Exception:
            pass
