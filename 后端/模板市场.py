...
''',
            "nodes.py": '''# ═══════════════════════════════════════════════════════
# 【模板类型：模型加载节点（model_loader）】本文件由 NodeCraft AI 模板生成
...
''',
            "nodes.py": '''# ═══════════════════════════════════════════════════════
# 【模板类型：工作流节点（workflow_node）】本文件由 NodeCraft AI 模板生成
...
// 后端节点定义（输入输出/执行逻辑）在 nodes.py，不在本文件。
// ═══════════════════════════════════════════════════════
...
ENTRY_TYPE_README_ROWS = {
    "canvas": "| 节点 / 自定义 Widget / 节点界面 | `web/js/extensions.js`（前端 Widget）与 `nodes.py`（节点定义） |",
    "sidebar": "| 侧边栏 / 侧边栏 Tab / 侧边栏界面 | `web/js/extensions.js` 中的 `registerSidebarTab` 块（title=标题，render=界面内容） |",
    "topMenu": "| 顶部菜单栏 / 菜单 / 菜单命令 | `web/js/extensions.js` 中的 `commands` 块（label=菜单文字，function=点击逻辑） |",
    "statusBar": "| 底部状态栏 / 状态栏 / 状态指示器 | `web/js/extensions.js` 中 `.comfyui-body-bottom` 相关代码（innerHTML=显示文字） |",
}

# 勾选项 → 对照说明
OPTION_README_ROWS = {
    "shortcuts": "| 快捷键与全局事件 / 快捷键 / 键盘事件 | `web/js/extensions.js` 末尾【勾选项：快捷键与全局事件】注释标记的 `keydown` 监听块 |",
    "settings": "| 设置面板 / 设置项 / 用户配置 | `web/js/extensions.js` 末尾【勾选项：设置面板】注释标记的 `addSetting` 调用（注册进 ComfyUI 自带设置对话框） |",
}

# 非 UI 扩展模板 → 对照说明
TEMPLATE_README_ROWS = {
    "basic_node": "| 节点 / 输入输出 / 节点分类 / 执行逻辑 | `nodes.py`（INPUT_TYPES=输入，RETURN_TYPES=输出，CATEGORY=分类，execute=逻辑） |",
    "image_processor": "| 图像处理 / 亮度 / 对比度 | `nodes.py` 的 `execute` 方法（PIL ImageEnhance） |",
    "model_loader": "| 模型加载 / 模型缓存 / 释放模型 | `nodes.py`（load_model=加载与缓存，unload=释放） |",
    "workflow_node": "| 条件分支 / 路由 / 开关 / 循环计数 | `nodes.py`（ConditionalRouter / ValueSwitch / LoopCounter） |",
}


def _构建功能对照表(template_id: str, entry_type: str, options: list) -> str:
    """生成 README 末尾的功能对照表，帮助对话智能体把用户口语术语映射到代码位置"""
    rows = []
    if template_id == "ui_extension":
        row = ENTRY_TYPE_README_ROWS.get(entry_type)
        if row:
            rows.append(row)
        for opt in options:
            row = OPTION_README_ROWS.get(opt)
            if row:
                rows.append(row)
    else:
        row = TEMPLATE_README_ROWS.get(template_id)
        if row:
            rows.append(row)
...
class TemplateMarket:
    ...
    def create_from_template(self, template_id: str, project_name: str,
        ...
        对照表 = _构建功能对照表(template_id, entry_type, options)
        if 对照表:
        ...
        # ── 创建标准目录结构（与"自定义插件"保持一致）──
        (target_dir / "节点").mkdir(exist_ok=True)
        (target_dir / "网页资源").mkdir(exist_ok=True)
        # 侧边栏/菜单/状态栏等 UI 扩展类模板额外创建"后端"目录，放置后端功能文件
        if template_id == "ui_extension":
            (target_dir / "后端").mkdir(exist_ok=True)
