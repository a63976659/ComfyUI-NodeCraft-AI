...
"""
    ...
    _EXCLUDE_NAMES = frozenset({
        '__pycache__', '.git', '.venv', 'node_modules', 'logs', '.ruff_cache',
        '.sync_progress.json', '.cipher_key', '.密钥',
    })
    # 排除的后缀（按路径分量后缀匹配，兼容 xxx.egg-info 目录）
    _EXCLUDE_SUFFIXES = ('.pyc', '.pyo', '.egg-info', '.db')

    def _should_exclude(self, file: Path, root: Path) -> bool:
        """判断文件是否应排除。

        按路径分量而非子串匹配：子串匹配下 '.git' 会误伤 '.github/' 与 '.gitignore'。
        """
        rel = file.relative_to(root)
        for part in rel.parts:
            if part in self._EXCLUDE_NAMES or part.endswith(self._EXCLUDE_SUFFIXES):
                return True
        return False
