...
_监控数据目录 = os.path.join(os.path.dirname(os.path.dirname(__file__)), "数据", "监控")


async def _持久化监控快照():
    """将监控指标快照到磁盘（由定时任务调用）"""
    try:
        os.makedirs(_监控数据目录, exist_ok=True)
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        filepath = os.path.join(_监控数据目录, f"snapshot_{timestamp}.json")
...
def _清理旧快照(max_files: int = 24):
    """清理旧快照（保留最近 24 个 = 24 小时）"""
    try:
        if not os.path.exists(_监控数据目录):
            return
        files = sorted(os.listdir(_监控数据目录))
        while len(files) > max_files:
            os.remove(os.path.join(_监控数据目录, files.pop(0)))
    except Exception:
        pass
