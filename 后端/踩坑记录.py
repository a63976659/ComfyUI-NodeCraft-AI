...
提供本地 JSON 存储的 FAQ CRUD 接口（纯本地，无云端同步）。
"""
import asyncio
import json
import time
...
class FaqEntry:
    ...
    updated_at: str = ""
    author: str = ""
    access_count: int = 0          # 被检索命中的次数（主动检索加权用）
    priority: float = 1.0          # 手动优先级调整（管理端可调，默认1.0）
...
def _delete_faq(faq_id: str) -> bool:
    """删除本地 FAQ 文件（纯本地，无云端同步）。

    返回值：是否成功（文件不存在时返回 False）。
    """
    original_file = _get_faq_file(faq_id)
    if not original_file.exists():
        return False
    try:
        original_file.unlink()
    except Exception as e:
        logger.exception(f"删除文件失败 {faq_id}: {e}")
        return False
    return True
...
async def 删除记录(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # P1-1：同步文件删除放入线程池
        deleted = await asyncio.to_thread(_delete_faq, faq_id)
        if not deleted:
            return web.json_response({"success": False, "error": "记录不存在"}, status=404)

        # 删除后刷新 FAQ 检索索引（延迟导入避免循环依赖）
        try:
            from 智能体.工具路由器 import invalidate_faq_index
            invalidate_faq_index()
        except Exception:
